...
class KitchenDrawer
  ...
  def merge_fork(forked_repository)
    ...
    if self.is_fork(forked_repository)
      ...
      self.merge_fork(forked_repository)
  ...
  def merge_fork(fork_repository)
    ...
    fork_owner = fork_owner[:owner][:login]
    ...
    unless Dir.exist?(File.join(@tmplocation, source_repo[:name]))
      begin
        ...
        gitty.clone({:quiet => false, 
                    ...
                    :timeout => @timeout}, 
                    source_repo[:ssh_url], 
                    File.join(@tmplocation, source_repo[:name]))
      rescue Git::Timeout
    ...
    #g = Git.clone(@dep_repository["clone_url"], :name => 'name', :path => tmp_dir)
    ...
    g.remotes.select {|x| x.name.start_with?(fork_owner)}.each do |branch|
      ...
      cloned_branch = self.client.login + "_" + branch.full.split("/")[2..-1].join("/")
      # using GH api here
